...
def main():
    generater = DataGenerater(Xpath=r'./data.csv', Ypath=r'./data.csv')
    ...
    preprocessor2.preprocess(pca_n_components=30)
    metric2 = Metrics()
...
class DataGenerater():
    ...
    def __init__(self, Xpath=r'./', Ypath=r'./'):
        # try:
        print('Loading Data')
        self.X = pd.read_csv(Xpath).drop(['label', 'file_flow_id'], axis=1)
        self.Y = pd.read_csv(Ypath)['label']
